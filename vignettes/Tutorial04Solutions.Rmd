...
- \newcommand{\md}{\,\mmd}
vignette: |
  %\VignetteIndexEntry{Tutorial 04: Uncertainty and integration (solutions)}
...
.tutorial <- FALSE
.solutions <- TRUE
begin_sol <- function(sol = .solutions) {if (!sol && !.all_sol) {"<!--\n"} else {"\n<hr />**Solution:**\n"}}
end_sol <- function(sol = .solutions) {if (!sol && !.all_sol) {"-->\n"} else {"\n<hr />\n"}}
...
* Download the `RMDemo.Rmd` file from Learn and upload to the Rstudio project and rename it to `Lab4.Rmd`
* During this lab, modify the `Lab4.Rmd` document and add new code and text commentary for the lab to the document.
...
Let $\theta=\log(\theta)$, so that $\lambda=\exp(\theta)$.
