...
a vignette in `StatCompLab` version 0.5.0 or later, with `vignette("Project01", "StatCompLab")`, or in pdf format on Learn. Updated 2010-02-20 to add a more efficient alternative for the importance sampler in part 2.
...
Note: You should ignore the cases where the $\theta$- or $\lambda$-intervals
extend below $0$ (for case 1, when the normal quantile $z$ has $\ol{y} < z^2/n$,
and for case 2, when $4\ol{y} < z^2/n$).
...
Let $N^\text{min}_j=\min(Y_{j,1}, Y_{j,2})$ be the smallest allowed value to sample,
...
$\xi_j=1/(1 + 3 N^\text{min}_j)$ when sampling, so that
...
defined as
$\log[w^{[k]}]=\log[p(\bm{N}^{[k]},\bm{Y})]-\log[\wt{p}(\bm{N}^{[k]}-\bm{N}^\text{min}_j)]$,
where $\wt{p}(\bm{N}^{[k]})=\prod_{j=1}^J p_{\pGeom(\xi_j)}(N_j^{[k]}-\bm{N}^\text{min}_j)$,
