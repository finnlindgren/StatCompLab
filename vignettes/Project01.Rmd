...
$$
a < \frac{\wh{\lambda}-\lambda}{\sqrt{\lambda/n}} < b
$$
...
With the help of the `dnorm`, define and document a function `log_like`, taking arguments `theta`, `x`, and `y`, that evaluates the observation log-likelihood for the model
 defined above.
...
`rmvnorm` and `dmvnorm` in the `matrixStats` package can be used to sample and
evaluate densities.

Define and document function `do_importance` taking arguments `N` (the number of samples
...
so that `sum(exp(log_weights))` is $1$. Use the `log_sum_exp` to compute the needed normalisation information.
